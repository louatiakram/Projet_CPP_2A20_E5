...
GDP::GDP(int ID_PRODUIT, QString NOM_PRODUIT, QDate DATE_PRODUCTION, QDate DATE_EXPIRATION, qreal PRIX, int QUANTITE)
{
   ...
   QSqlQueryModel *GDP::chercher(QString rech)
   {
       QSqlQueryModel *model= new QSqlQueryModel();
       model->setQuery("SELECT * FROM GESTION_PRODUIT WHERE ID_PRODUIT LIKE'%"+rech+"%' or NOM_PRODUIT  LIKE'%"+rech+"%' or DATE_PRODUCTION  LIKE'%"+rech+"%' or DATE_EXPIRATION  LIKE'%"+rech+"%' or PRIX  LIKE'%"+rech+"%' or QUANTITE LIKE'%"+rech+"%' ");
       return model;
   }
